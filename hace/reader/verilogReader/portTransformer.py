...
class PortTransformer(Transformer):
    ...
    def declaration(self, items):
        statements = []
        dir, t, signedness, range, variables, expression = items
        ...
        for variable in variables:
            port = Port(variable)
            port.setAll(dir, t, range, False if signedness is None else signedness.data == "signed")
            if expression:
                assert (
                    len(variables) == 1
