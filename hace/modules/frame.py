...
class Port:
    ...
    def setSigned(self, signed: bool):
        if signed is not None:
            ...
            if self.signed is not None and self.signed != signed:
                raise ValueError(
                    f"Signed mismatch: {self.signed} != {signed}, signal = {self.variable}"
                )
            self.signed = signed
            return True
        return False

    def setAll(self, direction: PortDirection, type: PortType, range: Range, signed: bool):
        self.setDirection(direction)
        self.setType(type)
        self.setRange(range)
        self.setSigned(signed)
